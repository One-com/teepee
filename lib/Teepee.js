/*global JSON, setTimeout, clearTimeout, setImmediate*/
...
Teepee.prototype.getAgent = function (protocol) {
    ...
    if (this.agent || this.Agent || (this.agentByProtocol && this.agentByProtocol[protocol])) {
...
Teepee.prototype.request = function (options, cb) {
    ...
    if (agent) {
        ...
        ['cert', 'key', 'ca'].forEach(function (key) {
            if (this[key]) {
    ...
    function dispatchRequest() {
        ...
        function handleRequestError(err) {
            ...
            if (numRetriesLeft > 0) {
                numRetriesLeft -= 1;
                setTimeout(function () {
                    that.emit('retriedRequest', { requestOptions: requestOptions, err: err, numRetriesLeft: numRetriesLeft });
                    dispatchRequest();
        ...
        currentRequest.on('error', handleRequestError).on('response', function (response) {
            ...
            if (response.statusCode >= 400) {
                ...
                if (numRetriesLeft > 0 && Array.isArray(retry) && (retry.indexOf(response.statusCode) !== -1 || retry.indexOf('httpError') !== -1)) {
                    response.on('error', function () {});
                    response.resume();
                    numRetriesLeft -= 1;
                    setTimeout(function () {
                        that.emit('retriedRequest', { requestOptions: requestOptions, err: responseError, numRetriesLeft: numRetriesLeft });
                        dispatchRequest();
                    }, retryDelayMilliseconds);
                    return;
                }
            ...
            if (cb) {
                ...
                if (responseBodyMustBeBuffered) {
                    ...
                    response.on('end', returnSuccessOrError).resume();
                }
            }
