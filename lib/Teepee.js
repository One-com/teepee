...
var EventEmitter = require('events').EventEmitter;
    ...
    var SelfRedirectError = createError({name: 'SelfRedirect'});
...
Teepee.prototype.subsidiary = function (config) {
    ...
    var that = this;
        var subsidiaryEmit = subsidiary.emit;
...
Teepee.prototype.request = function (url, options, cb) {
    ...
    var numRetriesLeft = options.streamRows ? 0 : typeof options.numRetries !== 'undefined' ? options.numRetries : this.numRetries || 0;
        var retryDelayMilliseconds = typeof options.retryDelayMilliseconds !== 'undefined' ? options.retryDelayMilliseconds : this.retryDelayMilliseconds || 0;
        var timeout = typeof options.timeout !== 'undefined' ? options.timeout : this.timeout;
        var username = typeof options.username !== 'undefined' ? options.username : this.username;
        var password = typeof options.password !== 'undefined' ? options.password : this.password;
        var body = typeof options.body !== 'undefined' ? options.body : this.body;
        var retry = typeof options.retry !== 'undefined' ? options.retry : this.retry || [];
        var rejectUnauthorized = typeof options.rejectUnauthorized !== 'undefined' ? options.rejectUnauthorized : this.rejectUnauthorized;
        var headers = {};
        // Gotcha: A query specified as a string overrides this.query
        var query = typeof options.query === 'string' ? options.query : { ...this.query, ...options.query };
        var method = options.method;
        var requestUrl = typeof options.path === 'string' ? options.path : options.url;
        var autoDecodeJson = typeof options.json !== 'undefined' ? options.json !== false : this.json !== false; // Defaults to true
