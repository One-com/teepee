...
Teepee.prototype.request = function (options, cb) {
    ...
    var matchUrl = url.match(
        ...
        // Fix syntax highlighting issue in sublime text with above regex with a single quote: '
    );
    if (!matchUrl) {
    ...
    var resolve, reject, onCancel;
    var promise = new Promise(function (_resolve, _reject, _onCancel) {
        resolve = _resolve;
        reject = _reject;
        onCancel = _onCancel;
    });
    ...
    if (this.preprocessRequestOptions) {
        this.preprocessRequestOptions(requestOptions, options, passError(reject, dispatchRequest));
    } else {
