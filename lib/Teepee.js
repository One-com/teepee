...
Teepee.prototype.request = function (options, cb) {
    ...
    function dispatchRequest() {
        ...
        currentRequest.once('error', handleRequestError).once('response', function handleResponse(response) {
            ...
            var emitSuccessOnEndIfNoCallback = false;
            ...
            if (response.statusCode === 301 || response.statusCode === 302) {
                ...
                body = null;
                responseBodyMustBeDisposedUnlessPiped = true;
                emitSuccessOnEndIfNoCallback = true;
            }
            ...
            if (cb) {
                ...
                } else if (emitSuccessOnEndIfNoCallback) {
                    // In the case of 304 and no callback and not calling 'then', we need to also call returnSuccessOrError when we're done
                    response.once('end', returnSuccessOrError);
                }
            }
            ...
            if (responseBodyMustBeBuffered) {
                ...
                response.on('data', function handleBodyChunk(responseBodyChunk) {
                    ...
                    eventEmitter.removeAllListeners(); // We already do this in other cases (eventEmitter.success and .error), and probably there should be no further events anyway
                    responseBodyChunks = null;
                    that = null;
                });
