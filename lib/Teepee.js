...
function Teepee(config) {
    ...
    if (typeof this.url === 'string') {
        _.extend(this, _.pick(new messy.HttpRequest(this.url), ['method', 'url']));
...
Teepee.prototype.getAgent = function () {
    if (!this.agent) {
        ...
        var Agent = this.Agent || require(this.url.indexOf('https:') === 0 ? 'https' : 'http').Agent;
...
Teepee.prototype.request = function (options, cb) {
    ...
    var numRetriesLeft = options.streamRows ? 0 : typeof options.numRetries !== 'undefined' ? options.numRetries : this.numRetries,
    ...
    var matchUrl = url.match(/^(https?:)\/\/(?:([^:@\/]+(?::[^@\/]+?))@)?((?:[a-z0-9](?:[\-a-z0-9]*[a-z0-9])?\.)*[a-z][\-a-z]*[a-z]|(?:(?:[0-9]|1?[0-9][0-9]|2[0-4][0-9]|25[0-5])\.){3}(?:[0-9]|1?[0-9][0-9]|2[0-4][0-9]|25[0-5]))(:\d{1,5})?(\/[\w\-\.~%!$&'\(\)*+,;=:@\/]*(?:\?[\w\-\.~%!$&'\(\)*+,;=:@\/?]*)?(?:#[\w\-\.~%!$&'\(\)*+,;=:@\/?#]*)?)?$/);
    if (!matchUrl) {
