...
Teepee.prototype.request = function (options, cb) {
    ...
    function cleanUp() {
        ...
        if (currentResponse) {
            ...
            currentResponse.on('error', function () {});
    ...
    function dispatchRequest() {
        ...
        currentRequest.once('error', handleRequestError).once('response', function (response) {
            ...
            if (cb) {
                ...
                if (responseError) {
                    if (responseBodyMustBeBuffered) {
                        response.once('responseBody', function () {
                            setImmediate(function () {
                                returnSuccessOrError();
                            });
                        });
                    } else {
                        response.once('end', returnSuccessOrError).resume();
                    }
                }
            }
            ...
            if (responseBodyMustBeBuffered) {
                ...
                }).once('error', returnSuccessOrError).once('end', function () {
                    currentRequest = null;
