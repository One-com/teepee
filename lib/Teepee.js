...
Teepee.prototype.preprocessQueryStringParameterValue = (queryStringParameterValue, queryStringParameterName) => queryStringParameterValue;
...
Teepee.prototype.expandUrl = function(url, requestOptions) {
  ...
  const that = this;
  let expandedUrl = url.replace(/\{((?:[^\{\}]+|\{\w+\})*)\}/g, ($0, placeholderName) => {
...
Teepee.prototype.request = function(url, options, cb) {
  ...
  // parse it separately:
  url = url.replace(/^([a-z+-]+:\/\/)([^:@/]+(?::[^@/]*?))@/i, ($0, before, _auth) => {
    auth = _auth;
    return before;
  });
      ...
      .once('response', function handleResponse(response) {
        ...
        function shouldRetryOnErrorStatusCode(statusCode) {
          return retry.some(retryEntry => {
            if (retryEntry === 'httpError' || retryEntry === statusCode) {
              ...
              if (
                retryEntry.replace(/x/g, ($0, index) => statusCodeString[index]) === statusCodeString
              ) {
                return true;
              }
            }
        ...
        if (response.statusCode === 301 || response.statusCode === 302) {
          if (numRetriesLeft > 0 && retry.indexOf('selfRedirect') !== -1) {
            const redirectTargetUrl = urlModule.resolve(
              url,
