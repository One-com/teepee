...
Teepee.prototype.preprocessQueryStringParameterValue = function(
...
Teepee.prototype.expandUrl = function(url, requestOptions) {
  ...
  var that = this;
  var expandedUrl = url.replace(/\{((?:[^\{\}]+|\{\w+\})*)\}/g, function(
    $0,
    placeholderName
  ) {
...
Teepee.prototype.request = function(url, options, cb) {
  ...
  var numRetriesLeft = options.streamRows
    ? 0
  ...
  // parse it separately:
  url = url.replace(/^([a-z+-]+:\/\/)([^:@/]+(?::[^@/]*?))@/i, function(
    $0,
    before,
    _auth
  ) {
      ...
      .once('response', function handleResponse(response) {
        ...
        function shouldRetryOnErrorStatusCode(statusCode) {
          return retry.some(function(retryEntry) {
            if (retryEntry === 'httpError' || retryEntry === statusCode) {
              ...
              if (
                retryEntry.replace(/x/g, function($0, index) {
                  return statusCodeString[index];
                }) === statusCodeString
              ) {
                return true;
              }
        ...
        if (response.statusCode === 301 || response.statusCode === 302) {
          if (numRetriesLeft > 0 && retry.indexOf('selfRedirect') !== -1) {
            var redirectTargetUrl = urlModule.resolve(
              url,
        ...
        var responseBodyMustBeBuffered =
